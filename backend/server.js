...
return raw.map(p=>{const name=clean(p.plan_name??p.name??p.plan??"");const networkName=normalizeDataNetwork(p.network??p.network_name??p.networkName??"");const id=Number(p.id??p.plan_id??p.planId??0);const price=Number(p.vendor_price??p.agent_price??p.user_price??p.price??p.amount??0);const sizeMatch=name.match(/(\d+(?:\.\d+)?)\s*(GB|MB)\b/i);const sizeMb=sizeMatch?(sizeMatch[2].toUpperCase()==="GB"?Math.round(Number(sizeMatch[1])*1024):Math.round(Number(sizeMatch[1]))):0;const rawValidity=clean(p.validity??p.validity_period??p.validityPeriod??p.duration??p.validityDuration??p.expiry??p.expiry_period??p.expiryPeriod??p.validity_days??""); const validityMatch=String(rawValidity).match(/(\d+(?:\.\d+)?)\s*(day|days|hour|hours|minute|minutes)\b/i); const validityDays=validityMatch&&/day/i.test(validityMatch[2])?Number(validityMatch[1]):(Number(p.validity_days)>0?Number(p.validity_days):0); return {network_name:networkName,name,plan_id:id,price,size_mb:sizeMb,validity_days:validityDays,validity:rawValidity,validity_period:rawValidity,duration:rawValidity,type:clean(p.type??"")};}).filter(p=>p.network_name===selected&&p.plan_id>0&&p.price>0&&p.name);}
...
byPlan.set(String(bundleId),{code:String(bundleId),bundle_id:bundleId,name:clean(plan.name||String(bundleId)),customer_price:customerPrice,provider_price:Number(providerPrice.toFixed(2)),network_name:network,service_id:null,size_mb:Number(plan.size_mb||0),validity_days:Number(plan.validity_days||0),validity:clean(plan.validity||plan.duration||"") ,duration:clean(plan.validity||plan.duration||"")});
}

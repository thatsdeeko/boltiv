...
function validPhone(phone){return /^0\d{10}$/.test(phone);}
function validAmount(amount){return Number.isFinite(amount)&&amount>0;}
...
await db(`UPDATE admins SET email=$1,password_hash=$2 WHERE id=$3`,[ADMIN_EMAIL,hashPassword(ADMIN_PASSWORD),result.rows[0].id]);
...
const r=await db(`SELECT id,type,service,amount,reference,provider_reference,status,phone,network,plan,meter_number,meter_type,smartcard_number,cable_package,provider,response_data,date,created_at FROM transactions WHERE user_id=$1 ORDER BY date DESC`,[userId]);
...
const w=await client.query(`UPDATE wallets SET balance=balance+$1,updated_at=NOW() WHERE user_id=$2 RETURNING balance`,[Number(payment.amount),payment.user_id]);
...
const r=await db(`SELECT a.id,a.email FROM admin_sessions s JOIN admins a ON a.id=s.admin_id WHERE s.token=$1 AND s.expires_at>NOW()`,[t]);

return r.rows[0]||null;
}

async function adminLogin(email,password){
if(!ADMIN_EMAIL||!ADMIN_PASSWORD)return{success:false,message:"Admin environment variables are not configured."};

const result=await db(`SELECT password_hash FROM admins WHERE LOWER(email)=LOWER($1)`,[ADMIN_EMAIL]);

const passwordHash=result.rows[0]?.password_hash||"";

if(email.toLowerCase()!==ADMIN_EMAIL.toLowerCase()||!verifyPassword(password,passwordHash)){
...
const providerResponse=await fetch(VTU_API_URL,{
method:"POST",
headers:{
"Content-Type":"application/json",
...
user_id,type,service,amount,reference,provider_reference,status,
...
const result=await client.query(`UPDATE wallets SET balance=balance-$1,updated_at=NOW() WHERE user_id=$2 AND balance >= $1 RETURNING balance`,[amount,userId]);
...
if(!debit.success){
return{success:false,statusCode:400,message:debit.message};
}
...
return{success:false,statusCode:502,message:"VTU provider connection failed."};
}

if(!providerResult.configured){
await refundWallet(userId,amount);
return{success:false,statusCode:503,message:providerResult.message};
}
...
if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
return send(res,400,{success:false,message:"A valid email is required."});
}

if(!Number.isFinite(amount)||amount<100){
return send(res,400,{success:false,message:"Minimum wallet funding amount is ₦100."});
}
...
if(!validPhone(phone)){
return send(res,400,{success:false,message:"Enter a valid Nigerian phone number."});
}

if(!network){
return send(res,400,{success:false,message:"Network is required."});
}

if(!validAmount(amount)){
return send(res,400,{success:false,message:"Enter a valid airtime amount."});
}
...
return send(res,result.statusCode||200,result);
}
...
const phone=clean(b.phone);
const network=clean(b.network);
const plan=clean(b.plan||b.planId||b.product);
const amount=Number(b.amount);
...
if(!plan){
return send(res,400,{success:false,message:"Data plan is required."});
}

if(!validAmount(amount)){
return send(res,400,{success:false,message:"Invalid data plan amount."});
}
...
const b=await body(req);
const userId=clean(b.userId);
const provider=clean(b.provider||b.disco);
const meterNumber=clean(b.meterNumber||b.meter);
const meterType=clean(b.meterType||b.type||"prepaid");
const amount=Number(b.amount);

if(!userId)return send(res,400,{success:false,message:"User ID is required."});

if(!provider){
return send(res,400,{success:false,message:"Electricity provider is required."});
}

if(!meterNumber){
return send(res,400,{success:false,message:"Meter number is required."});
}

if(!validAmount(amount)){
return send(res,400,{success:false,message:"Enter a valid electricity amount."});
}
...
const b=await body(req);
const userId=clean(b.userId);
const provider=clean(b.provider||b.cable);
const smartcardNumber=clean(b.smartcardNumber||b.smartcard||b.iuc);
const cablePackage=clean(b.cablePackage||b.package||b.plan);
const amount=Number(b.amount);

if(!userId)return send(res,400,{success:false,message:"User ID is required."});

if(!provider){
return send(res,400,{success:false,message:"Cable provider is required."});
}

if(!smartcardNumber){
return send(res,400,{success:false,message:"Smartcard/IUC number is required."});
}

if(!cablePackage){
return send(res,400,{success:false,message:"Cable package is required."});
}

if(!validAmount(amount)){
return send(res,400,{success:false,message:"Invalid cable package amount."});
}
...
const r=await db(`SELECT id,user_id,type,service,amount,reference,provider_reference,status,phone,network,plan,meter_number,meter_type,smartcard_number,cable_package,provider,response_data,date,created_at FROM transactions ORDER BY date DESC LIMIT 100`);
...
// BOLTIV SERVER.JS — PART 2/2 — PostgreSQL + Paystack + Admin + Wallet + VTU — FINAL CLEAN VERSION

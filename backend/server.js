...
const send=(r,s,d)=>{r.writeHead(s,{"Content-Type":"application/json","Access-Control-Allow-Origin":"*","Access-Control-Allow-Methods":"GET,POST,PUT,OPTIONS","Access-Control-Allow-Headers":"Content-Type,Authorization"});r.end(JSON.stringify(d))};
const readBody=req=>new Promise((ok,no)=>{let b="";req.on("data",x=>b+=x);req.on("end",()=>{try{ok(b?JSON.parse(b):{})}catch(e){no(e)}});req.on("error",no)});
...
let r=await db(`SELECT u.* FROM user_sessions s JOIN users u ON u.id=s.user_id WHERE s.token=$1 AND s.expires_at>NOW()`,[h.slice(7).trim()]);
return r.rows[0]||null;
};

const userSession=async id=>{
let t=token();
await db(`INSERT INTO user_sessions(token,user_id,expires_at) VALUES($1,$2,NOW()+INTERVAL '7 days')`,[t,id]);
return t;
};
...
let r=await db(`SELECT u.id FROM password_resets p JOIN users u ON u.id=p.user_id WHERE p.token=$1 AND p.used=FALSE AND p.expires_at>NOW()`,[t]);
...
let e=await db(`SELECT id FROM users WHERE LOWER(email)=LOWER($1) AND user_id<>$2`,[email,id]);
if(e.rows.length)return{success:false,message:"Email is already in use."};
let r=await db(`UPDATE users SET name=$1,phone=$2,email=$3,initials=$4,updated_at=NOW() WHERE user_id=$5 RETURNING user_id,name,phone,email,initials`,[name,phone,email,initials(name),id]);
...
let r=await db(`SELECT password_hash FROM users WHERE user_id=$1`,[id]);
...
let r=await fetch(`https://api.paystack.co${path}`,{...opt,headers:{Authorization:`Bearer ${PAYSTACK_SECRET_KEY}`,"Content-Type":"application/json",...(opt.headers||{})}});
...
let reference=ref(),r=await paystack("/transaction/initialize",{method:"POST",body:JSON.stringify({email,amount:String(Math.round(amount*100)),currency:"NGN",reference,callback_url:`${FRONTEND_URL}/payment-success.html`,metadata:{userId,service:"BOLTIV Wallet Funding"}})});
...
if(!PAYSTACK_SECRET_KEY)return{success:false,message:"Paystack is not configured on the server."};
let p=(await db(`SELECT * FROM payments WHERE reference=$1`,[reference])).rows[0];
...
let r=await paystack(`/transaction/verify/${encodeURIComponent(reference)}`,{method:"GET"});
if(!r.data.status)return{success:false,message:r.data.message||"Unable to verify payment."};
let x=r.data.data;
if(x.status!=="success"){
await db(`UPDATE payments SET status=$1 WHERE reference=$2`,[x.status,reference]);
return{success:false,message:`Payment status: ${x.status}`,status:x.status};
}
...
let w=await c.query(`UPDATE wallets SET balance=balance+$1,updated_at=NOW() WHERE user_id=$2 RETURNING balance`,[Number(p.amount),p.user_id]);
...
try{
await c.query("BEGIN");
let w=await c.query(`SELECT balance FROM wallets WHERE user_id=$1 FOR UPDATE`,[id]);
...
let r=await c.query(`UPDATE wallets SET balance=balance-$1,updated_at=NOW() WHERE user_id=$2 RETURNING balance`,[amount,id]);
...
// BOLTIV SERVER.JS PART 1 END
async function adminAuth(req){
let h=req.headers.authorization||"";
if(!h.startsWith("Bearer "))return null;
let r=await db(`SELECT a.id,a.email FROM admin_sessions s JOIN admins a ON a.id=s.admin_id WHERE s.token=$1 AND s.expires_at>NOW()`,[h.slice(7).trim()]);
return r.rows[0]||null;
}

async function adminLogin(email,password){
let r=await db(`SELECT * FROM admins WHERE LOWER(email)=LOWER($1)`,[email]);
...
let r=await db(`SELECT u.user_id,u.name,u.phone,u.email,COALESCE(w.balance,0) balance,u.created_at,u.updated_at FROM users u LEFT JOIN wallets w ON w.user_id=u.user_id ORDER BY u.created_at DESC LIMIT 100`);
return r.rows.map(x=>({...x,balance:Number(x.balance)}));
}

async function adminTransactions(){
let r=await db(`SELECT id,user_id,type,service,amount,reference,status,date FROM transactions ORDER BY date DESC LIMIT 100`);
return r.rows.map(x=>({...x,amount:Number(x.amount)}));
}

async function adminPayments(){
let r=await db(`SELECT reference,user_id,email,amount,status,credited,created_at,credited_at FROM payments ORDER BY created_at DESC LIMIT 100`);
return r.rows.map(x=>({...x,amount:Number(x.amount)}));
}
...
let b=await readBody(req),r=await createUser(String(b.name||"").trim(),String(b.phone||"").trim(),String(b.email||"").trim().toLowerCase(),String(b.password||""));
return send(res,r.success?201:400,r);
}

if(req.method==="POST"&&p==="/api/auth/login"){
let b=await readBody(req),r=await loginUser(String(b.email||"").trim().toLowerCase(),String(b.password||""));
...
let b=await readBody(req),id=String(b.userId||"").trim();
if(!id)return send(res,400,{success:false,message:"User ID is required."});
await makeWallet(id);
let w=await wallet(id);
return send(res,200,{success:true,userId:id,balance:w.balance});
}

if(req.method==="GET"&&p==="/api/wallet"){
let id=u.searchParams.get("userId");
if(!id)return send(res,400,{success:false,message:"User ID is required."});
let w=await wallet(id);
...
let b=await readBody(req),id=String(b.userId||"").trim(),email=String(b.email||"").trim(),amount=Number(b.amount);
...
let r=u.searchParams.get("reference");
...
let x=await userAuth(req);
if(!x)return send(res,401,{success:false,message:"Unauthorized."});
let b=await readBody(req),amount=Number(b.amount);
...
let b=await readBody(req),r=await adminLogin(String(b.email||"").trim().toLowerCase(),String(b.password||""));
return send(res,r.success?200:401,r);
}

if(req.method==="GET"&&p==="/api/admin/me"){
let x=await adminAuth(req);
if(!x)return send(res,401,{success:false,message:"Unauthorized."});
...
let w=await db(`SELECT COUNT(*)::int count,COALESCE(SUM(balance),0) balance FROM wallets`),t=await db(`SELECT COUNT(*)::int count FROM transactions`),pmt=await db(`SELECT COUNT(*)::int count,COALESCE(SUM(CASE WHEN status='success' THEN amount ELSE 0 END),0) successful FROM payments`);
...
if(!await adminAuth(req))return send(res,401,{success:false,message:"Unauthorized."});
return send(res,200,{success:true,users:await adminUsers()});
}

if(req.method==="GET"&&p==="/api/admin/transactions"){
if(!await adminAuth(req))return send(res,401,{success:false,message:"Unauthorized."});
return send(res,200,{success:true,transactions:await adminTransactions()});
}

if(req.method==="GET"&&p==="/api/admin/payments"){
if(!await adminAuth(req))return send(res,401,{success:false,message:"Unauthorized."});
return send(res,200,{success:true,payments:await adminPayments()});
}

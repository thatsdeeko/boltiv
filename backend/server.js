...
if(adminPricing){
  ...
  // is stored, always resolve to provider_discount so a stale record or a direct API call
  ...
  return {markup_mode:"provider_discount",markup_pct:Number.isFinite(discountPct)?Math.min(100,Math.max(0,discountPct)):0,markup_fixed:0,service_fee:Number.isFinite(serviceFee)?Math.max(0,serviceFee):0};
}
